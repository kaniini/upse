		...
		if (spuCtrl & 0x80)	// -> reverb on? oki
		{
			...
			INPUT_SAMPLE_R >>= (16 - 8);
			{
				...
				ACC0 = ((g_buffer(rvb.ACC_SRC_A0) * rvb.ACC_COEF_A) >> 15) +
					((g_buffer(rvb.ACC_SRC_B0) * rvb.ACC_COEF_B) >> 15) + ((g_buffer(rvb.ACC_SRC_C0) * rvb.ACC_COEF_C) >> 15) + ((g_buffer(rvb.ACC_SRC_D0) * rvb.ACC_COEF_D) >> 15);
				ACC1 = ((g_buffer(rvb.ACC_SRC_A1) * rvb.ACC_COEF_A) >> 15) +
					((g_buffer(rvb.ACC_SRC_B1) * rvb.ACC_COEF_B) >> 15) + ((g_buffer(rvb.ACC_SRC_C1) * rvb.ACC_COEF_C) >> 15) + ((g_buffer(rvb.ACC_SRC_D1) * rvb.ACC_COEF_D) >> 15);
