 ...
 *
 * Filename: upse_recompiler_x64_impl.c
 * Purpose: libupse: x64 dynamic recompiler implementation
 *
 * Copyright (c) 2007 William Pitcock <nenolod@sacredspiral.co.uk>
 * Portions copyright (c) 1999-2002 Pcsx Team
 * Portions copyright (c) 2004 "Xodnizel"
 * Portions copyright (c) 2004-2007 Pcsx2 Team
 * Portions copyright (c) 2007 Ryan Schultz
 *
...
#include "upse-recompiler-x64-primitives.h"
...
static uptr *recRAM;	/* and the ptr to the blocks here */
static uptr *recROM;	/* and here */
...
static void iFlushReg(int reg) {
	if (IsConst(reg)) {
		MOV32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[reg], iRegs[reg].k);
	}
...
static void iRet() {
	/* store cycle */
	count = (pc - pcold)/4;
	ADD32ItoM((uptr)&upse_r3000_cpu_regs.cycle, count);
	//if (resp) ADD64ItoR(RSP, resp);
...
static void SetBranch() {
	branch = 1;
	upse_r3000_cpu_regs.code = PSXMu32(pc);
	pc+=4;

	if (iLoadTest() == 1) {
		iFlushRegs();
		MOV32ItoM((uptr)&upse_r3000_cpu_regs.code, upse_r3000_cpu_regs.code);
		/* store cycle */
		count = (pc - pcold)/4;
		ADD32ItoM((uptr)&upse_r3000_cpu_regs.cycle, count);
	...
	recBSC[upse_r3000_cpu_regs.code>>26]();

	iFlushRegs();
	MOV32MtoR(EAX, (uptr)&target);
	MOV32RtoM((uptr)&upse_r3000_cpu_regs.pc, EAX);
...
static void iJump(u32 branchPC) {
	...
	iFlushRegs();
	MOV32ItoM((uptr)&upse_r3000_cpu_regs.pc, branchPC);
	CALLFunc((uptr)psxBranchTest);
	/* store cycle */
	count = (pc - pcold)/4;
	ADD32ItoM((uptr)&upse_r3000_cpu_regs.cycle, count);
	//if (resp) ADD32ItoR(RSP, resp);
	...
	CMP32ItoM((uptr)&upse_r3000_cpu_regs.pc, branchPC);
	j8Ptr[0] = JE8(0);

	RET();

	x86SetJ8(j8Ptr[0]);
	MOV64MtoR(RAX, (uptr)PC_REC(branchPC));
	//TEST32RtoR(RAX, RAX);
	AND64RtoR(RAX,RAX);
	j8Ptr[1] = JNE8(0);
...
static void iDumpRegs() {
	...
	printf("%lx %lx\n", upse_r3000_cpu_regs.pc, upse_r3000_cpu_regs.cycle);
	for (i=0; i<4; i++) {
		for (j=0; j<8; j++)
			printf("%lx ", upse_r3000_cpu_regs.GPR.r[j*i]);
		printf("\n");
	}
}
...
void iDumpBlock(char *ptr) {
	...
	SysPrintf("dump1 %x:%x, %x\n", upse_r3000_cpu_regs.pc, pc, upse_r3000_cpu_regs.cycle);

	for (i = upse_r3000_cpu_regs.pc; i < pc; i+=4)
...
static void rec##f() { \
	iFlushRegs(); \
	MOV32ItoM((uptr)&upse_r3000_cpu_regs.code, (u32)upse_r3000_cpu_regs.code); \
	MOV32ItoM((uptr)&upse_r3000_cpu_regs.pc, (u32)pc); \
	CALLFunc((uptr)psx##f); \
/*	branch = 2; */\
}
...
#define RECRAM_POS (&psxMemWrite32 - (0x280000*sizeof(void*)) - 0xc000000)
#define RECMEM_POS (RECRAM_POS - RECMEM_SIZE)

static int recInit() {
	int i;

	psxRecLUT = (uptr**) malloc(0x010000 * sizeof(void*));

	recMem = mmap(RECMEM_POS,
	//recMem = mmap(0,
		RECMEM_SIZE,
		PROT_EXEC | PROT_WRITE | PROT_READ, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	memset(recMem, 0, RECMEM_SIZE);

	recRAM = mmap(RECRAM_POS,
		0x280000*sizeof(void*),
		PROT_WRITE | PROT_READ, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
	//recRAM = (uptr*) malloc(0x200000 * sizeof(void*));
	//recROM = (uptr*) malloc(0x080000 * sizeof(void*));
	recROM = &recRAM[0x200000];

	if (recRAM == NULL || recROM == NULL || recMem == NULL || psxRecLUT == NULL) {
		SysMessage("Error allocating memory"); return -1;
	}
	memset(recRAM, 0, 0x200000 * sizeof(void*));
	memset(recROM, 0, 0x080000 * sizeof(void*));

	for (i=0; i<0x80; i++) psxRecLUT[i + 0x0000] = &recRAM[((i & 0x1f) << 16)];
	memcpy(psxRecLUT + 0x8000, psxRecLUT, 0x80 * sizeof(void*));
	memcpy(psxRecLUT + 0xa000, psxRecLUT, 0x80 * sizeof(void*));

	for (i=0; i<0x08; i++) psxRecLUT[i + 0xbfc0] = &recROM[(i << 16)];
...
static void recShutdown() {
	...
	//free(recRAM);
	munmap(recMem, 0x280000*sizeof(void*));
	//free(recROM);
	x86Shutdown();
}
...
__inline static void execute() {
	...
	p =	(char*)PC_REC(upse_r3000_cpu_regs.pc);
	if (p != NULL) 
		recFunc = (void (**)()) p;
...
static void recClear(u32 Addr, u32 Size) {
	memset((void*)PC_REC(Addr), 0, Size * sizeof(void*) * 4);
}
...
//	iFlushRegs();
	...
	if (_Rs_ == _Rt_) {
		if (IsConst(_Rt_)) {
			...
			if (_Imm_ == 1) {
				INC32M((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
			} else if (_Imm_ == -1) {
				DEC32M((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
			} else if (_Imm_) {
				ADD32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_], _Imm_);
			}
		}
	} else {
		if (IsConst(_Rs_)) {
			...
			MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
			if (_Imm_ == 1) { 
				INC32R(EAX);
			} else if (_Imm_ == -1) {
				DEC32R(EAX);
			...
			MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_], EAX);
		}
	}
}
...
//	iFlushRegs();
	...
	if (_Rs_ == _Rt_) {
		if (IsConst(_Rt_)) {
			...
			AND32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_], _ImmU_);
		}
	} else {
		if (IsConst(_Rs_)) {
			MapConst(_Rt_, iRegs[_Rs_].k & _ImmU_);
...
//	iFlushRegs();
	...
	if (_Rs_ == _Rt_) {
		if (IsConst(_Rt_)) {
			...
			OR32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_], _ImmU_);
		}
	} else {
		if (IsConst(_Rs_)) {
			MapConst(_Rt_, iRegs[_Rs_].k | _ImmU_);
...
//	iFlushRegs();
	...
	if (_Rs_ == _Rt_) {
		if (IsConst(_Rt_)) {
			...
			XOR32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_], _ImmU_);
		}
	} else {
		if (IsConst(_Rs_)) {
			MapConst(_Rt_, iRegs[_Rs_].k ^ _ImmU_);
...
// Rt = Imm << 16
	...
	MapConst(_Rt_, upse_r3000_cpu_regs.code << 16);
}
#endif
//End of Load Higher .....
...
//	iFlushRegs();
	...
	if (IsConst(_Rs_) && IsConst(_Rt_)) {
		...
		if (_Rt_ == _Rd_) {
			if (iRegs[_Rs_].k == 1) {
				INC32M((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_]);
			} else if (iRegs[_Rs_].k == -1) {
				DEC32M((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_]);
			} else if (iRegs[_Rs_].k) {
				ADD32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], iRegs[_Rs_].k);
			}
		} else {
			MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
			if (iRegs[_Rs_].k == 1) { 
				INC32R(EAX);
			...
			MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
		}
		...
		if (_Rs_ == _Rd_) {
			if (iRegs[_Rt_].k == 1) {
				...
				ADD32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], iRegs[_Rt_].k);
			}
		} else {
			MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
			if (iRegs[_Rt_].k == 1) { 
		...
		if (_Rs_ == _Rd_) { // Rd+= Rt
			MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
			ADD32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
		} else if (_Rt_ == _Rd_) { // Rd+= Rs
			MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
			ADD32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
		} else { // Rd = Rs + Rt
			MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
			ADD32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
			MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
		}
	}
}
...
//	iFlushRegs();
	...
	if (IsConst(_Rs_) && IsConst(_Rt_)) {
		...
		SUB32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
		MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
	} else if (IsConst(_Rt_)) {
		iRegs[_Rd_].state = ST_UNK;
...
//	iFlushRegs();
	...
	if (IsConst(_Rs_) && IsConst(_Rt_)) {
		...
		if (_Rd_ == _Rt_) { // Rd&= Rs
			AND32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], iRegs[_Rs_].k);
		} else {
			MOV32ItoR(EAX, iRegs[_Rs_].k);
			AND32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
			MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
		}
	} else if (IsConst(_Rt_)) {
		iRegs[_Rd_].state = ST_UNK;
		...
		if (_Rd_ == _Rs_) { // Rd&= kRt
			AND32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], iRegs[_Rt_].k);
		} else { // Rd = Rs & kRt
			MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
		...
		if (_Rs_ == _Rd_) { // Rd&= Rt
			MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
			AND32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
		} else if (_Rt_ == _Rd_) { // Rd&= Rs
			MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
			AND32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
		} else { // Rd = Rs & Rt
...
//	iFlushRegs();
	...
	if (IsConst(_Rs_) && IsConst(_Rt_)) {
		...
		MOV32ItoR(EAX, iRegs[_Rs_].k);
		OR32MtoR (EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
		MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
	} else if (IsConst(_Rt_)) {
		iRegs[_Rd_].state = ST_UNK;
...
//	iFlushRegs();
	...
	if (IsConst(_Rs_) && IsConst(_Rt_)) {
		...
		MOV32ItoR(EAX, iRegs[_Rs_].k);
		XOR32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
		MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
	} else if (IsConst(_Rt_)) {
		iRegs[_Rd_].state = ST_UNK;
...
//	iFlushRegs();
	...
	if (IsConst(_Rs_) && IsConst(_Rt_)) {
		...
		MOV32ItoR(EAX, iRegs[_Rs_].k);
		CMP32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
		SETL8R   (EAX);
		AND32ItoR(EAX, 0xff);
		MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
	} else if (IsConst(_Rt_)) {
...
//	iFlushRegs();
	...
	if ((IsConst(_Rs_) && iRegs[_Rs_].k == 0) ||
		...
		XOR32RtoR(EAX, EAX);
		MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.n.lo, EAX);
		MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.n.hi, EAX);
		return;
	}
	...
	if (IsConst(_Rt_)) {
		...
		IMUL32M  ((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
	}
	MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.n.lo, EAX);
	MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.n.hi, EDX);
}
...
//	iFlushRegs();
	...
	if (IsConst(_Rt_)) {
		...
		MUL32M   ((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
	}
	MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.n.lo, EAX);
	MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.n.hi, EDX);
}
...
//	iFlushRegs();
	...
	if (IsConst(_Rt_)) {
		...
		MOV32MtoR(ECX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
		CMP32ItoR(ECX, 0);
		j8Ptr[0] = JE8(0);
	}
	if (IsConst(_Rs_)) {
...
static void SetArg_OfB(x86IntRegType arg) {
	if (IsConst(_Rs_))
		...
		MOV32MtoR(arg, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
		if (_Imm_)
			ADD32ItoR(arg, _Imm_);
	}
}
		...
		if (t == 0x1f80) {
			switch (addr) {
				...
				case 0x1f801810:
					...
					MOV64ItoR(RAX, (uptr)GPU_readData);
					CALL64R(RAX);
				...
				case 0x1f801814:
					...
					MOV64ItoR(RAX, (uptr)GPU_readStatus);
					CALL64R(RAX);
...
// Rt = Rt Merge mem[Rs + Im]
	...
	if (_Rt_) {
		...
		if (IsConst(_Rs_)) MOV32ItoR(EDX, iRegs[_Rs_].k + _Imm_);
			...
			MOV32MtoR(EDX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
		...
		if (IsConst(_Rt_)) {
			...
			MOV32MtoR(EDX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
		}
	...
	if (IsConst(_Rs_)) {
		...
		if ((t & 0x1fe0) == 0) {
			for (i=0; i<count; i++, code++, addr+=4) {
				...
				MOV32MtoR(EAX, (uptr)&psxM[addr & 0x1fffff]);
				MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_fRt_(*code)], EAX);
			}
			return;
		}
		if (t == 0x1f80 && addr < 0x1f801000) {
...
//	ADD32ItoR(ESP, 4);
	...
	for (i=0; i<count; i++, code++) {
		if (_fRt_(*code)) {
			...
			MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_fRt_(*code)], EDX);
		}
	...
	if (IsConst(_Rs_)) {
		...
		if ((t & 0x1fe0) == 0) {
			if (IsConst(_Rt_)) {
				...
				MOV8MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
...
//		SysPrintf("unhandled w8 %x\n", addr);
	...
	if (IsConst(_Rt_)) {
		...
		MOV32MtoR(X86ARG2, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
	}
	//iPushOfB();
	SetArg_OfB(X86ARG1);
	...
	if (IsConst(_Rs_)) {
		...
		if ((t & 0x1fe0) == 0) {
			if (IsConst(_Rt_)) {
				...
				MOV16MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
		...
		if (t == 0x1f80) {
			if (addr >= 0x1f801c00 && addr < 0x1f801e00) {
				if (IsConst(_Rt_)) {
					...
					//PUSH64M((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
					MOV32MtoR(X86ARG2, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
				}
				//PUSHI  (addr);
				MOV64ItoR(X86ARG1, addr);
				//CALLFunc  ((uptr)SPU_writeRegister);
				MOV64ItoR(RAX, (uptr)SPU_writeRegister);
				CALL64R(RAX);
#ifndef __WIN32__
				//resp+= 8;
#endif
			...
			switch (addr) {
				...
				case 0x1f801810:
					if (IsConst(_Rt_)) {
						...
						//PUSH64M((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
						MOV32MtoR(X86ARG1, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
					}
					//CALLFunc((uptr)GPU_writeData);
					MOV64ItoR(RAX, (uptr)GPU_writeData);
					CALL64R(RAX);
#ifndef __WIN32__
					//resp+= 4;
#endif
				...
				case 0x1f801814:
					...
					MOV64ItoR(RAX, (uptr)GPU_writeStatus);
					CALL64R(RAX);
#ifndef __WIN32__
					//resp+= 4;
#endif
		...
		if ((t & 0x1fe0) == 0) {
			for (i=0; i<count; i++, code++, addr+=4) {
				if (IsConst(_fRt_(*code))) {
					...
					MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_fRt_(*code)]);
					MOV32RtoM((uptr)&psxM[addr & 0x1fffff], EAX);
				}
			}
			return;
...
//	ADD32ItoR(ESP, 4);
	...
	for (i=0, code = PSXM(pc); i<count; i++, code++) {
		if (IsConst(_fRt_(*code))) {
			...
			MOV32MtoR(EDX, (uptr)&upse_r3000_cpu_regs.GPR.r[_fRt_(*code)]);
		}
...
//	iFlushRegs();
	...
	if (IsConst(_Rt_) && IsConst(_Rs_)) {
		...
		MOV32MtoR(ECX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
		SHL32CLtoR(EAX);
		MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
	} else {
		iRegs[_Rd_].state = ST_UNK;
...
// Rd = Hi
	...
	iRegs[_Rd_].state = ST_UNK;
	MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.n.hi);
	MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
}
...
// Hi = Rs
	...
	if (IsConst(_Rs_)) {
		MOV32ItoM((uptr)&upse_r3000_cpu_regs.GPR.n.hi, iRegs[_Rs_].k);
	} else {
		MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
		MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.n.hi, EAX);
	}
...
// Rd = Lo
	...
	iRegs[_Rd_].state = ST_UNK;
	MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.n.lo);
	MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rd_], EAX);
}
...
// Lo = Rs
	...
	if (IsConst(_Rs_)) {
		MOV32ItoM((uptr)&upse_r3000_cpu_regs.GPR.n.lo, iRegs[_Rs_].k);
	} else {
		MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
		MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.n.lo, EAX);
	}
...
//	iFlushRegs();
	...
	CMP32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_], 0);
	j32Ptr[4] = JL32(0);
...
//	iFlushRegs();
	...
	if (IsConst(_Rs_)) {
		if ((s32)iRegs[_Rs_].k < 0) {
			MOV32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[31], pc + 4);
			iJump(bpc); return;
		} else {
			iJump(pc+4); return;
		}
...
//	iFlushRegs();
	...
	if (_Rs_ == _Rt_) {
		...
		if (IsConst(_Rs_) && IsConst(_Rt_)) {
			...
			CMP32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_], iRegs[_Rs_].k);
		} else if (IsConst(_Rt_)) {
			CMP32ItoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_], iRegs[_Rt_].k);
		} else {
			MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rs_]);
			CMP32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_]);
		}
...
// Rt = Cop0->Rd
	...
	iRegs[_Rt_].state = ST_UNK;
	MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.CP0.r[_Rd_]);
	MOV32RtoM((uptr)&upse_r3000_cpu_regs.GPR.r[_Rt_], EAX);
}
...
// Cop0->Rd = Rt
	...
	if (IsConst(_Rt_)) {
		switch (_Rd_) {
			case 12:
				MOV32ItoM((uptr)&upse_r3000_cpu_regs.CP0.r[_Rd_], iRegs[_Rt_].k);
				break;
			case 13:
				MOV32ItoM((uptr)&upse_r3000_cpu_regs.CP0.r[_Rd_], iRegs[_Rt_].k & ~(0xfc00));
				break;
			default:
				MOV32ItoM((uptr)&upse_r3000_cpu_regs.CP0.r[_Rd_], iRegs[_Rt_].k);
				break;
		...
		MOV32RtoM((uptr)&upse_r3000_cpu_regs.CP0.r[_Rd_], EAX);
	}
	...
	if (_Rd_ == 12 || _Rd_ == 13) {
		...
		if (_Rd_ == 12)
			OR32ItoM((uptr)&upse_r3000_cpu_regs.interrupt, 0x80000000);
		if (branch == 0) {
			branch = 2;
			iRet();
		}
...
static void recRFE() {
	MOV32MtoR(EAX, (uptr)&upse_r3000_cpu_regs.CP0.n.Status);
	MOV32RtoR(ECX, EAX);
	AND32ItoR(EAX, 0xfffffff0);
	AND32ItoR(ECX, 0x3c);
	SHR32ItoR(ECX, 2);
	OR32RtoR (EAX, ECX);
	MOV32RtoM((uptr)&upse_r3000_cpu_regs.CP0.n.Status, EAX);
...
static void recHLE() {
	...
	//MOV32ItoR(EAX, (uptr)psxHLEt[upse_r3000_cpu_regs.code & 0xffff]);
	//CALL32R(EAX);
	CALLFunc((uptr)psxHLEt[upse_r3000_cpu_regs.code & 0xffff]);
	branch = 2;
	iRet();
}
...
static void (*recBSC[64])() = {
	...
	recNULL   , recNULL  , recNULL, recNULL , recNULL, recNULL, recNULL, recNULL,
	recNULL   , recNULL  , recNULL, recHLE  , recNULL, recNULL, recNULL, recNULL
};
...
static void recRecompile() {
	...
	if (((uptr)x86Ptr - (uptr)recMem) >= (RECMEM_SIZE - 0x10000))
		recReset();

	x86Align(32);
	ptr = x86Ptr;

	*PC_REC(upse_r3000_cpu_regs.pc) = (uptr)x86Ptr;
	pc = upse_r3000_cpu_regs.pc;
	pcold = pc;
	...
	for (count=0; count<500;) {
		p = (char *)PSXM(pc);
		if (p == NULL) recError();
		upse_r3000_cpu_regs.code = *(u32 *)p;
/*
		if ((upse_r3000_cpu_regs.code >> 26) == 0x23) { // LW
			int i;
			u32 code;
		...
		if ((upse_r3000_cpu_regs.code >> 26) == 0x2b) { // SW
			int i;
			u32 code;
	...
	MOV32ItoM((uptr)&upse_r3000_cpu_regs.pc, pc);
	iRet();
}
