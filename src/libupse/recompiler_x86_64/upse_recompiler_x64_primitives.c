 ...
 *
 * Filename: upse_recompiler_x64_primitives.c
 * Purpose: libupse: x64 dynamic recompiler implementation; primitive insns
 *
...
#error This code only runs on x64 compatible CPU cores.
...
void MOV32ItoM(uptr to, u32 from ) 
{
	...
	if (to <= 0xffffffff)	{
		...
		assert((sptr)MEMADDR(to,8) <= 0x7fffffff && (sptr)MEMADDR(to,8) >= -0x7fffffff);
		ModRM( 0, 0, DISP32 );
		write32( MEMADDR(to, 8) );
	}
	write32( from ); 
